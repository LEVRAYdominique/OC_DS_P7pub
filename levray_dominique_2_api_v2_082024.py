...
========================================================================
Note : Cette API est la version 2 de l'API developpée pour le projet n°7

Pour exécuter cette API localement : uvicorn levray_dominique_2_api_v2_082024:app --reload
...
# Chargement des données depuis le fichier ZIP créés lors de la modélisation
...
async def prediction(request: Request):
    '''Obtenir une prédiction à partir d'une ligne de données complète (114 colonnes)'''

    # Récupérer les données envoyées
    data_dict = await request.json()

    # Convertir le dictionnaire en dataframe
    data_df        = pd.DataFrame(columns=data_dict.keys())     # Créer un dataframe vide
    data_df.loc[0] = list(data_dict.values())                   # Ajouter une ligne
    
    # Retraite les données car le format doit être le suivant : "type=nom de colonne": "valeur" en string
    for col in data_df.columns:
        # Les clefs sont sous la forme type=nom de colonne
        new_type, new_key = col.split("=")          

        # Renomme la colonne pour supprimer le type et le signe =
        data_df = data_df.rename(columns={col: new_key})

        if (new_type=="bool")|(new_type=="int64")|(new_type=="float64"):
            # Conserver la valeur texte pour le traitement des boolean
            oldvalue=data_df.loc[0, new_key]
            # Convertir le type de la colonne avec le type demandée
            data_df[new_key] = data_df[new_key].astype(new_type)
            # Traite le cas particulier des boolean
            if (new_type=="bool"):
                data_df.loc[0, new_key] = (True if oldvalue=="True" else False)
       
    # Prépare les données
    temp_y = data_df['TARGET']
    temp_X = data_df.drop(columns='TARGET')

    # Faire la prédiction
    temp_y_proba = model.predict_proba(temp_X)[:, 1][0]
    temp_y_pred  = np.where(temp_y_proba >= BEST_THRESHOLD, 1, 0)

    return {"y_proba":str(temp_y_proba), "y_pred": str(temp_y_pred)}
